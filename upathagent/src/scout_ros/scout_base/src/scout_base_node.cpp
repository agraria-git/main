...
int main(int argc, char **argv) {
  ...
  if (proto == ProtocolVersion::AGX_V1) {
    ...
    ROS_INFO("Fallo protocolo");
    std::cout << "Detected protocol: UNKONWN2" << std::endl;
    ROS_ERROR("Detected protocol: UNKONWN"); //new elk
    return -1;
  }
  if (robot == nullptr)
  ...
  private_node.param<bool>("simulated_robot", messenger.simulated_robot_,
                           false);
/* -------------------------------- */
  private_node.param<bool>("tf_pub", messenger.tf_pub_,
                           true);
/* -------------------------------- */
