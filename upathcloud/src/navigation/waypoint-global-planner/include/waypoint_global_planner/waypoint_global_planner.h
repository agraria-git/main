...
class WaypointGlobalPlanner : public nav_core::BaseGlobalPlanner
{
  public:
    ...
    //TOADD
    void magicCallback(const geometry_msgs::PointStamped& cords);
    
    // Actualizacion UTM
    void currentPoseCallback(const geometry_msgs::PoseStamped& cords);

    void updatePath(std::vector<geometry_msgs::PoseStamped>& Wpath);
    int nearestSegment(geometry_msgs::Point P0, geometry_msgs::Point P1, geometry_msgs::Point P2, geometry_msgs::Point pos);
    bool pointInSegment(geometry_msgs::Point p, geometry_msgs::Point PIni, geometry_msgs::Point PFin, float m, float b);

    void viaCallback(const nav_msgs::PathConstPtr& via);
  ...
  private:
    ...
    ros::Subscriber viapoints_sub_;
     //TOADD
    // ros::Subscriber magic_sub_;  //!< subscriber of external input path
    // Actualizacion UTM
    ros::Subscriber base_link_sub_;
    ...
    geometry_msgs::PoseStamped originPos;
